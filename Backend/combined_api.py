...
def tts():
    logger.info("Received request at /tts")
    data = request.get_json()
    if not data or "raw" not in data:
        logger.warning("Missing 'raw' in request")
    ...
    logger.info("Generating audio from provided script")
    client = genai.Client(
        api_key=api_key,
    )
    ...
    contents = [
        types.Content(
            role="user",
            parts=[
                types.Part.from_text(text=data["raw"]),
            ],
        ),
    ]
    generate_content_config = types.GenerateContentConfig(
    ...
    for chunk in client.models.generate_content_stream(
        ...
        if chunk.candidates[0].content.parts[0].inline_data and chunk.candidates[0].content.parts[0].inline_data.data:
            ...
            logger.info("Audio chunk received in /tts")
            break

    if audio_bytes is None:
        logger.error("No audio generated in /tts")
    ...
    audio_io.seek(0)
    logger.info("Returning audio file from /tts")
    return send_file(
        audio_io,
        mimetype="audio/wav",
        as_attachment=True,
...
def generate_full():
    ...
    logger.info("Generating audio from script in /generate_full")
